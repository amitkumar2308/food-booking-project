...
const Spend = ({ instance }) => {
    ...
    const handleSpendClick = async () => {
        try {
            ...
            // const args = [spendTo];
            const args = ["ak_VfYAqeatjV6jgLf2QLZ38yxK19PYqxFnqp81qwdUWasriqe6u"];
    ...
    return (
        <div className="max-w-md mx-auto bg-white p-6 rounded-md shadow-md">
            {/* <div className="mb-4">
                ...
                />
						...
						<div className="mb-4">
        <label className="block text-gray-700 text-sm font-bold mb-2">Enter dish name...</label>
        <input
          className="appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
          // value={spendAmount}
          // onChange={(e) => setSpendAmount(e.target.value)}
        />
            ...
            {spendPromise && (
                <div className="mt-4">
                    <div className="font-bold text-lg mb-2">Spend result</div>
                    <p className="text-gray-700">{`Transiction id : ${spendPromise} .`}</p>
                </div>
            )}
        </div>
    );
